...
#define SPI_WRITE_MASK(x) (uint16_t)(x | 0x8000)  // Sets MSB for write
#define SPI_READ_MASK(x)  (uint16_t)(x & ~0x8000) // Clears MSB for read

extern SPI_HandleTypeDef hspi2;//SPI handler (defined in STM32CubeMX)after u createe it in Mx it will be in main.c
